...
renderTemplate (res, req, "nsfw.ejs");
});
...
app.get("/bekliyor", (req, res) => {
  
renderTemplate(res, req, "bekleme.ejs")
});
...
app.get("/api/:botID", checkAuth, (req, res) => {
...
renderTemplate(res, req, "botekle.ejs")
 ...
 app.get("/:botID/vote", checkAuth, (req, res) => {
  var id = req.params.botID
  renderTemplate(res, req, "oyla.ejs", {id}) 
});
...
if (db.has('botlar')) {
    if (Object.keys(db.fetch('botlar')).includes(ID) === true) return res.redirect('/addbot/error')
}
...
client.channels.get(client.ayarlar.kayıt).send(`<@${req.user.id}> added bot **${sistem.username}#${sistem.discriminator}**`)

if (client.users.has(req.user.id) === true) {
  client.users.get(req.user.id).send(`Your bot **${sistem.username}** has been added and waiting for approval.`)
}
...
renderTemplate(res, req, "panel.ejs")
  ...
  app.get("/yetkili/all", (req, res) => {
    if(!client.yetkililer.includes(req.user.id) ) return res.redirect('/yetkili/hata')

renderTemplate(res, req, "y-panel-all.ejs")
});
    app.get("/yetkili/certification", (req, res) => {
    if(!client.yetkililer.includes(req.user.id) ) return res.redirect('/yetkili/hata')

renderTemplate(res, req, "y-panel-certificate.ejs")
});
      app.get("/yetkili/pending", (req, res) => {
    if(!client.yetkililer.includes(req.user.id) ) return res.redirect('/yetkili/hata')

renderTemplate(res, req, "y-panel-pendings.ejs")
});
        app.get("/yetkili/approve", (req, res) => {
    if(!client.yetkililer.includes(req.user.id) ) return res.redirect('/yetkili/hata')

renderTemplate(res, req, "y-panel-apporve.ejs")
});
...
client.channels.get(client.ayarlar.kayıt).send(`<@${req.user.id}> edited **${sistem.username}#${sistem.discriminator}**`)
...
client.channels.get(client.ayarlar.rapor).send(`\`${req.user.username}#${req.user.discriminator}\` adlı kullanıcı \`${db.fetch(`botlar.${req.params.botID}.isim`)}\` adlı botu raporladı! \n**Sebep:** \`${ayar['mesaj-1']}\``)
}
if(ayar['mesaj-2']) {
db.push(`botlar.${req.params.botID}.raporlar`, JSON.parse(`{ "rapor":"${ayar['mesaj-2']}" }`))
client.channels.get(client.ayarlar.rapor).send(`<@${req.user.id}> reported **${db.fetch(`botlar.${req.params.botID}.isim`)}**\n**Reason:** \`${ayar['mesaj-2']}\``)
}
...
var saat = `${new Date().getHours() + 6}:${new Date().getMinutes()}:${new Date().getSeconds()}`
...
if (db.has(`oylar.${id}.${user}`) === true) {
  ...
  db.set(`oylar.${id}.${user}`, saat)
    if(db.has(`botlar.${id}.webhookurl`)) {
      fetch(db.fetch(`botlar.${id}.webhookurl`), {
        method: 'post',
        body:    {"userid":req.user.id, "botid": id,"type":"voted","username":req.user.username},
...
client.channels.get(client.ayarlar.vote).send(`<@${req.user.id}> voted bot named **${db.fetch(`botlar.${req.params.botID}.isim`)}**`)
res.redirect('/bot/'+req.params.botID)
...
client.channels.get(client.ayarlar.kayıt).send(`<@${db.fetch(`botlar.${id}.sahipid`)}>'s bot named **${db.fetch(`botlar.${id}.isim`)}** has been approved by <@${req.user.id}>`)
if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Your bot **${db.fetch(`botlar.${id}.isim`)}** has been apporved.\nhttps://astariuslist.gq/bot/${db.fetch(`botlar.${id}.id`)}`)
...
client.channels.get(client.ayarlar.kayıt).send(``)
  
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(``)
  }
});
...
app.post("/botyonetici/reddet/:botID", checkAuth, (req, res) => {
  ...
  client.channels.get(client.ayarlar.kayıt).send(`<@${db.fetch(`botlar.${id}.sahipid`)}>'s bot named **${db.fetch(`botlar.${id}.isim`)}** was rejected by the <@${req.user.id}> authority.`)
  
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Your bot **${db.fetch(`botlar.${id}.isim`)}** was rejected by the <@${req.user.id}> authority.\n\nReason: ${req.body['red-sebep']}`)
  }
db.delete(`botlar.${id}`) 
db.delete(`kbotlar.${req.user.id}.${id}`)
...
res.redirect("/yetkili")
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Your bot **${db.fetch(`botlar.${id}.isim`)}** gived certificate!`)
  }
...
app.post("/botyonetici/sertifikaverme/:botID", checkAuth, (req, res) => {
  ...
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Your bot **${db.fetch(`botlar.${id}.isim`)}** certificate was not issued.\n Reason: ${req.body['sertifika-red-sebep']}`)
  }
...
app.post("/botyonetici/sertifikasil/:botID", checkAuth, (req, res) => {
  ...
  if (client.users.has(db.fetch(`botlar.${id}.sahipid`)) === true) {
  client.users.get(db.fetch(`botlar.${id}.sahipid`)).send(`Your bot **${db.fetch(`botlar.${id}.isim`)}** certificate deleted.\n${req.body['sertifika-sil-sebep']}`)
  }
...
app.get("/api/botlar/:botID/oylar/:kullaniciID", (req, res) => {
  ...
  if (db.has('botlar')) {
      if (Object.keys(db.fetch('botlar')).includes(id) === false) {
     res.json({
       hata: 'A bot with the ID you entered exists in the system!'
     });
   }
  }
 
   res.json({
     "vote": db.has(`oylar.${id}.${userr}`) ? true : null
   });

});
  
  app.post("/api/webhookseturl/:ID",(req,res) =>{
    ...
    if(!req.body["url"]) {
      res.json({"hata": "Url Not writed.", "ok":"false"})
      return;
    } else if(Object.keys(db.fetch('botlar')).includes(id)) {
  db.set(`botlar.${id}.webhookurl`,req.body["url"]);
    res.json({"ok":"true"})
    }
    })
